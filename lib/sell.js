...
async function getPipelines() {
  // List all pipelines in account
  // Sell v2 supports /v2/pipelines. Request a generous page size.
  const json = await sellFetch('/v2/pipelines?per_page=200', { method: 'GET' });
  return (json?.items || []).map((x) => x.data).filter(Boolean);
}
